...
long accumulator = 0;
  ...
  PIDValue = Error[0] * motorPTerm;            // start with proportional gain
  //PIDValue += motorITerm * accumulator;   // add integral gain and Error accumulation
  //Serial.println("Acc: " + String(accumulator));
  PIDValue += motorITerm * accumulator;
...
void measureAndCalculate() {
  ...
  unsigned long echoTimeout = micros();
